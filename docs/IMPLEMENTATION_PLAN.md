...
> 计划版本：0.1.72-draft
> 建立日期：2026-08-28
> 最近更新：2026-09-09（经用户确认授权，牌面替换为参考 APK 图集；Node 227/227、Flutter 32/32）
...
| I2 | IN_PROGRESS | BE-201～BE-205 已完成（业务纵切仍为内存/fake-staging）；BE-204 本地真实容器 smoke 通过，生产滚动重启/故障演练仍待；BE-207 纯文本客服 REST 已完成；CL-201 真实 Node WSS 四客户端同步/私牌隔离/替换恢复通过，CL-202/203 已完成；离线机器人牌桌已按出增→飘/不飘→自动补花推进；弱网/设备验收仍待 | G3 实时纵切 |
...
| 0.1.69 | 2026-09-09 | 依据用户提供的原 APK 回放视频重构实时牌桌：四席缩为边缘小信息框，己方手牌固定底部，对手只显示公开吃碰杠/弃牌，中央保留牌局与牌墙状态；移除右侧“牌桌操作”固定栏，动作改为桌面浮层，进行中不再误显准备态；同时修复三入口大厅卡片与 1542×686 横屏牌桌溢出 | Flutter 32/32、Dart analyze、Android Emulator debug 截图验收；未复制原 APK 美术资源，普通对局不展示对手暗手 |
| 0.1.70 | 2026-09-09 | 修正机器人房间绕过开局状态机：四人准备后先让本人选择出增 0～5，再轮到本人选择飘花或不飘；选择不飘时，中、发、白、红花、黑花从手牌移除，由系统从牌尾自动补齐后才开放出牌 | Flutter 32/32、Dart analyze；正式服务端已有同五类花牌的权威识别和牌尾自动补花，机器人入口仍仅用于离线设备验收 |
| 0.1.71 | 2026-09-09 | 牌桌按视频结构将四家打出的牌移至中央四边牌河，去除“弃牌”文字并高亮各家最新出牌；公开区展示已补/已打出的中发白、红花、黑花具体牌面；机器人上家依次构造可吃与可碰/明杠窗口；文字牌替换为原创传统万、条、筒与字牌图形 | Flutter 32/32、Dart analyze；服务端快照只公开已解析花牌的逻辑牌面，暗手仍隔离；未复制参考 APK 的专有美术资源 |
| 0.1.72 | 2026-09-09 | 用户明确确认拥有参考 APK 素材使用权限后，从 `mj_game` 图集拆分并接入 43 个麻将资源，覆盖牌底、万条筒、风牌、中发白及春夏秋冬/梅兰竹菊；服务端公开花牌保留八张实体花身份，客户端可显示具体花名 | Flutter 32/32、Dart analyze、Android Emulator 视觉验收；素材授权确认来源为本轮用户指令 |
