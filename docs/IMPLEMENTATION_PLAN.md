...
> 计划版本：0.1.2-draft
> 建立日期：2026-08-28
> 最近更新：2026-09-07（BE-304 出牌响应窗口、过牌轮转与候选动作识别，Node 148/148、Flutter 16/16）
...
| 测试 | 当前 148 个 Node 测试通过；Dart 协议/原生 transport/四客户端 fake 夹具脚本通过；Flutter POC 16/16 通过；`verify:real` 与 `verify:multi-instance` 均通过 | 继续增加真实进程滚动重启、弱网和平台级故障测试 |
...
| BE-304 | 动作合法性和优先级 | BE-303、DEC-RULE-002/007 | draw/discard/chi/peng/gang/hu/pass（以签字动作集为准） | 非回合/非法牌/过期动作拒绝；IN_PROGRESS（draw/discard 回合阶段、手牌归属、服务端取牌、公开弃牌、三家顺序响应/过牌及重启恢复已实现；服务端可识别碰/明杠/下家吃候选，待候选的私有投影、动作执行与签字优先级） |
...
| 宿松规则 | IN_PROGRESS | 用户已确认采用参考 APK 8931 规则；开房配置、花数/胡型/杠开、服务端计分、候选 144 张牌墙、发牌、连续补花、摸出牌、保留 14 张流局和过牌响应窗口已编码，私密牌墙已接入持久化和按玩家脱敏视图；补花方向与动作优先级仍缺旧服样本 | 实现碰/杠/胡候选的玩家私有投影与执行，再形成 ≥20 个 golden cases |
