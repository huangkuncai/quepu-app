...
> 计划版本：0.1.2-draft
> 建立日期：2026-08-28
> 最近更新：2026-09-07（BE-304/305 不必胡过圈状态，Node 167/167、Flutter 16/16）
...
| BE-304 | 动作合法性和优先级 | BE-303、DEC-RULE-002/007 | draw/discard/chi/peng/gang/hu/pass（以签字动作集为准） | 非回合/非法牌/过期动作拒绝；IN_PROGRESS（draw/discard、手牌归属、三家顺序响应/过牌、按玩家私有候选投影及权威吃/碰/明杠/暗杠/巴杠、抢杠胡、标准胡/七对/清一色/混一色/碰碰胡、自摸、点炮、必胡自动裁决和不必胡过圈均已实现；当前优先级为胡 > 碰/明杠 > 吃；待其余特殊胡型） |
...
| 宿松规则 | IN_PROGRESS | 用户已确认采用参考 APK 8931 规则；开房配置、花数/胡型/杠开、服务端计分、候选 144 张牌墙、发牌、连续补花、摸出牌、保留 14 张流局、响应窗口、吃/碰/明杠/暗杠/巴杠/抢杠胡、标准胡/七对/清一色/混一色/碰碰胡、自摸/点炮/必胡/过圈与一炮多响收集已编码，私密牌墙已接入持久化和按玩家脱敏视图；补花方向、过圈解除边界、三西识别及其余特殊胡型仍缺旧服样本 | 补三西与剩余特殊胡型识别，并建立 golden cases |
...
下一次规则纵切从 **BE-306 三西识别与剩余特殊胡型 golden cases** 开始；基础设施并行待办仍为 I2 的真实设备/服务验收。顺序如下：
...
9. 已完成标准胡/七对候选、点炮/自摸、一炮多响收集、必胡自动结算、下家多候选吃牌、暗杠、巴杠、抢杠胡及不必胡过圈；下一步实现三西识别和可由权威状态证明的剩余特殊胡型，未签字部分继续 fail-closed。
