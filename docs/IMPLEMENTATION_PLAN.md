...
> 计划版本：0.1.2-draft
> 建立日期：2026-08-28
> 最近更新：2026-09-07（BE-301/303 牌墙接入 Room、私密持久化与玩家脱敏重连，Node 139/139、Flutter 16/16）
...
| 测试 | 当前 139 个 Node 测试通过；Dart 协议/原生 transport/四客户端 fake 夹具脚本通过；Flutter POC 16/16 通过；`verify:real` 与 `verify:multi-instance` 均通过 | 继续增加真实进程滚动重启、弱网和平台级故障测试 |
...
| BE-303 | 发牌、补花、牌墙、庄轮转 | DEC-RULE-002/003 | round state、dealer、wall、deadline | 固定 seed 重现；流局边界正确；IN_PROGRESS（候选开局算法已接入 Room/RoomActor/RoomService；全量私牌和牌墙仅进入原子持久化快照，玩家快照只含本人手牌并共享公共 hash；待实现真实补花取牌和跨局庄轮转） |
...
| 宿松规则 | IN_PROGRESS | 用户已确认采用参考 APK 8931 规则；开房配置、花数/胡型/杠开、服务端计分及候选 144 张牌墙/开局发牌已编码，私密牌墙已接入持久化和按玩家脱敏视图；补花方向与动作优先级仍缺旧服样本 | 实现版本化补花取牌与手牌守恒，再形成 ≥20 个 golden cases |
