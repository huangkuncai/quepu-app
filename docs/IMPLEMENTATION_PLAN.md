...
> 计划版本：0.1.2-draft
> 建立日期：2026-08-28
> 最近更新：2026-09-08（规则基线 `.5` 补齐多人抢杠、过圈与动作冲突优先级证据；Node 221/221、Flutter 24/24）
...
| 0.1.59 | 2026-09-08 | 补齐同一弃牌上吃、碰、胡并发冲突测试：即使低优先级响应先到，服务端仍在窗口收齐后按胡 > 碰/明杠 > 吃裁决；同步将 DEC-RULE-002 更新为已确认 | `be-303-susong-wall.test.js`；`npm run check`（Node 221/221） |
