...
当前质量门：`npm run check` 为 Node 213/213；Flutter 24/24，静态分析无问题。
...
仍需用“旧服连续录屏 + 对局日志/结算截图”或规则负责人书面确认解除：

1. `susong-144-candidate-v1` 的精确牌组构成、开局 53 张发牌批次与剩余 14 张保留区口径。
2. 吃、碰、杠、胡同时出现时除现有“胡 > 碰/明杠 > 吃”以外的全部冲突优先级。
3. “春夏秋冬/梅兰竹菊”参与四对子时是否按红花/黑花两组累计对子单位；当前实现采用五类特殊牌合并对子单位的书面描述。
