...
当前所有已确认纵切均已实现并通过自动化。正式规则仍有两项会改变牌墙或不可逆积分，必须由规则负责人书面确认：

1. 牌组签字：是否正式采用当前候选的 144 张，即万/条/筒各 36、东南西北各 4、中发白各 4、红花/黑花各 4；中发白和红黑花均为摸到后补出的花牌。
2. 无花果放冲：APK 原文“无花果放冲（点炮）即为一索”是否表示“无花果玩家作为放炮者时，本次赢家的胡牌基础档强制为一索”；若是，强飘的飘花玩家与不强飘尚未抓到花的玩家是否都适用，一炮多响是否对每个赢家分别适用。

在第 2 项确认前，当前已确认实现只处理“无花果玩家自己不能接炮、只能自摸且自摸一索”，不会猜测放炮者无花时的升档。
