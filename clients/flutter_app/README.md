...
这是 G1 的 Flutter 候选壳。默认使用 `FakeTransport` 离线演示，也可通过 `SUSONG_WSS_URL` 和 `SUSONG_REST_URL` 连接隔离的开发服务，用于验证登录、大厅、房号加入、牌桌、断线同步和纯文本客服。
...
真实开发链路运行方式见 [协议消费说明](../../docs/client/PROTOCOL_CONSUMER.md)。仓库根目录执行 `npm run verify:client-real`，可自动验证四个真实 WSS 客户端的房间流程、私牌隔离、重连恢复，以及同一登录会话创建/查询客服 REST 工单。
